import {
  ...
  watchWithFilter,
  whenever
} from "./chunk-OBSJLYAN.js";
import {
  del,
  isVue2,
  set
} from "./chunk-7Q2OEYUD.js";
import "./chunk-IKFQJB6E.js";
import "./chunk-HYQHY4CN.js";
import {
  computed,
  customRef,
  getCurrentInstance,
  ...
  watch,
  watchEffect
} from "./chunk-CIFZ4UBC.js";
import "./chunk-N4GPHMAN.js";
import {
  __async,
...
function computedAsync(evaluationCallback, initialState, optionsOrRef) {
  ...
  let counter = 0;
  watchEffect((onInvalidate) => __async(this, null, function* () {
    if (!started.value)
      return;
    counter++;
    ...
    try {
      const result = yield evaluationCallback((cancelCallback) => {
        onInvalidate(() => {
          if (evaluating)
...
function useAsyncState(promise, initialState, options) {
  ...
  function execute(delay2 = 0, ...args) {
    return __async(this, null, function* () {
      ...
      if (delay2 > 0)
        yield promiseTimeout(delay2);
      const _promise = typeof promise === "function" ? promise(...args) : promise;
      try {
        const data = yield _promise;
        state.value = data;
        isReady.value = true;
      } catch (e) {
        error.value = e;
...
function useDevicesList(options = {}) {
  ...
  function update() {
    return __async(this, null, function* () {
      if (!isSupported)
        return;
      devices.value = yield navigator.mediaDevices.enumerateDevices();
  ...
  function _start() {
    return __async(this, null, function* () {
      if (!isSupported || stream.value)
        return;
      stream.value = yield navigator.mediaDevices.getDisplayMedia(constraint);
      return stream.value;
    });
  }
  function _stop() {
...
function useFetch(url, ...args) {
  ...
  const execute = (throwOnFailed = false) => __async(this, null, function* () {
    var _a22;
    loading(true);
    error.value = null;
    ...
    if (options.beforeFetch)
      Object.assign(context, yield options.beforeFetch(context));
    if (isCanceled || !fetch) {
      loading(false);
    ...
    return new Promise((resolve, reject) => {
      ...
      })).then((fetchResponse) => __async(this, null, function* () {
        response.value = fetchResponse;
        statusCode.value = fetchResponse.status;
        responseData = yield fetchResponse[config.type]();
        if (options.afterFetch && statusCode.value >= 200 && statusCode.value < 300)
          ({ data: responseData } = yield options.afterFetch({ data: responseData, response: fetchResponse }));
        data.value = responseData;
        if (!fetchResponse.ok)
          throw new Error(fetchResponse.statusText);
        responseEvent.trigger(fetchResponse);
        return resolve(fetchResponse);
      })).catch((fetchError) => __async(this, null, function* () {
        let errorData = fetchError.message || fetchError.name;
        if (options.onFetchError)
          ({ data: responseData, error: errorData } = yield options.onFetchError({ data: responseData, error: fetchError, response: response.value }));
        data.value = responseData;
...
function useFileSystemAccess(options = {}) {
  ...
  function open() {
    return __async(this, arguments, function* (_options = {}) {
      if (!isSupported)
        return;
      const [handle] = yield window2.showOpenFilePicker(__spreadValues$8(__spreadValues$8({}, unref(options)), _options));
  ...
  function create() {
    return __async(this, arguments, function* (_options = {}) {
      if (!isSupported)
        return;
      fileHandle.value = yield window2.showSaveFilePicker(__spreadValues$8(__spreadValues$8({}, unref(options)), _options));
      data.value = void 0;
      yield updateFile();
      yield updateData();
    });
  ...
  function save() {
    return __async(this, arguments, function* (_options = {}) {
      ...
      if (data.value) {
        const writableStream = yield fileHandle.value.createWritable();
        yield writableStream.write(data.value);
  ...
  function updateFile() {
    return __async(this, null, function* () {
      var _a2;
      file.value = yield (_a2 = fileHandle.value) == null ? void 0 : _a2.getFile();
    });
  }
  function updateData() {
    return __async(this, null, function* () {
      ...
      if (unref(dataType) === "Text")
        data.value = yield (_a2 = file.value) == null ? void 0 : _a2.text();
      if (unref(dataType) === "ArrayBuffer")
        data.value = yield (_b2 = file.value) == null ? void 0 : _b2.arrayBuffer();
...
function useMediaControls(target, options = {}) {
  ...
  const togglePictureInPicture = () => {
    return new Promise((resolve, reject) => {
      usingElRef(target, (el) => __async(this, null, function* () {
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = (..._0) => __async(this, [..._0], function* (overrideOptions = {}) {
    if (isSupported) {
...
function useStorageAsync(key, initialValue, storage, options = {}) {
  ...
  function read(event) {
    return __async(this, null, function* () {
      if (!storage || event && event.key !== key)
        return;
      try {
        const rawValue = event ? event.newValue : yield storage.getItem(key);
        if (rawValue == null) {
          data.value = rawInit;
          if (writeDefaults && rawInit !== null)
            yield storage.setItem(key, yield serializer.write(rawInit));
        } else {
          data.value = yield serializer.read(rawValue);
        }
      } catch (e) {
        onError(e);
      }
  ...
  if (storage) {
    watchWithFilter(data, () => __async(this, null, function* () {
      try {
        if (data.value == null)
          yield storage.removeItem(key);
        else
          yield storage.setItem(key, yield serializer.write(data.value));
      } catch (e) {
        onError(e);
      }
    }), {
...
var useWakeLock = (options = {}) => {
  ...
  function request(type) {
    return __async(this, null, function* () {
      if (!isSupported)
        return;
      wakeLock = yield navigator.wakeLock.request(type);
      isActive.value = !wakeLock.released;
    });
  }
  function release() {
...
var useWebNotification = (defaultOptions2 = {}) => {
  ...
  const requestPermission = () => __async(void 0, null, function* () {
    if (!isSupported)
      return;
  ...
  const show = (overrides) => __async(void 0, null, function* () {
    if (!isSupported)
      return;
  ...
  tryOnMounted(() => __async(void 0, null, function* () {
    if (isSupported)
      yield requestPermission();
  }));
