...
function generate(sourceLang, templateAst, isVue2, cssScopedClasses = [], htmlToTemplate, isScriptSetup, searchTexts) {
    ...
    function visitNode(node, parentEl) {
        if (node.type === 1 /* ELEMENT */) {
            ...
            for (let i = 0; i < node.branches.length; i++) {
                ...
                if (branch.condition?.type === 4 /* SIMPLE_EXPRESSION */) {
                    tsCodeGen.addText(` (`);
        ...
        function writeEvents(node) {
            ...
            for (const prop of node.props) {
                ...
                function appendExpressionNode(prop, jsChildNode) {
                    if (prop.exp?.type === 4 /* SIMPLE_EXPRESSION */) {
    ...
    function writeProps(node, forRemainStyleOrClass, mode) {
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* DIRECTIVE */
                ...
                if (prop.arg?.type === 4 /* SIMPLE_EXPRESSION */) {
                ...
                else if (prop.exp?.constType === 3 /* CAN_STRINGIFY */) {
    ...
    function writeImportSlots(node, parentEl) {
        for (const prop of node.props) {
            if (prop.type === 7 /* DIRECTIVE */
                ...
                let isStatic = true;
                if (prop.arg?.type === 4 /* SIMPLE_EXPRESSION */ && prop.arg.content !== '') {
    ...
    function writeSlots(node) {
        ...
        function getSlotNameExp() {
            for (const prop2 of node.props) {
                if (prop2.type === 7 /* DIRECTIVE */ && prop2.name === 'bind' && prop2.arg?.type === 4 /* SIMPLE_EXPRESSION */ && prop2.arg.content === 'name') {
                    if (prop2.exp?.type === 4 /* SIMPLE_EXPRESSION */) {
